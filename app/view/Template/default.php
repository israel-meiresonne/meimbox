...
<head>
    ...
    <?= $API_ON ? '<meta name="google-site-verification" content="aIjZLmnjdEIh9dRdLUSzmDentOZboTY8VYr5r6FWnv0" />' : '' ?>
    <?= self::META_DEVICE ?>
    <?= self::STYLE_W3SCHOOL ?>
    ...
    <script>
        ...
        const API_ON = (<?= $API_ON ? 1 : 0 ?> === 1) ? true : false ;
    ...
    <?= $API_ON ? $this->getAPIBaseCodes(Facebook::class, Google::class) : $this->getADMCodes() ?>
</head>

<body>
    <?= $headerContent ?>
    <div class="template-content">
        <?= $API_ON ? $this->generateAPIEvents(Facebook::class, Google::class) : '' ?>
        <?= $content ?>
